...
class TestCurrencyDisplay:
    """Verify prices display with NT$ prefix."""

    def test_currency_on_product_card(self, page: Page):
        """Product cards on homepage show NT$ prefix."""
        page.goto(FRONTEND_URL)
        page.wait_for_selector("a[href^='/product/']", timeout=10000)
        price_el = page.locator(".text-lg.font-bold.text-red-600").first
        text = price_el.text_content()
        assert text is not None
        assert "NT$" in text, f"Expected NT$ in price, got: {text}"

    def test_currency_on_detail_page(self, page: Page):
        """Product detail page shows NT$ prefix."""
        page.goto(FRONTEND_URL)
        page.wait_for_selector("a[href^='/product/']", timeout=10000)
        page.locator("a[href^='/product/']").first.click()
        page.wait_for_url(re.compile(r"/product/\d+"))
        page.wait_for_selector(".text-3xl.font-bold.text-red-600", timeout=10000)
        price_el = page.locator(".text-3xl.font-bold.text-red-600")
        text = price_el.text_content()
        assert text is not None
        assert "NT$" in text, f"Expected NT$ in price, got: {text}"
...
class TestUpdateButton:
    """Update Now button tests."""

    def test_update_button_visible(self, page: Page):
        """'Update Now' button is visible on homepage."""
        page.goto(FRONTEND_URL)
        page.wait_for_selector("a[href^='/product/']", timeout=10000)
